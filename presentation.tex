...
\frame{
	...
	\begin{itemize}
		...
		\item Hadamard and Toffoli gates are in $\spn\mathcal{G}_{XZ}$
	\end{itemize}
}

\frame{
	\frametitle{Local Hamiltonian is QBP-hard}
	Let $C=U_T\ldots U_1$ be a circuit with only Hadamard and Toffoli gates. Let $\ket{x}$ be the initial state.
	\pause
	Goal: construct $H$ so
